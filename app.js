...
function chartContext(){
  ...
  const planetNames=Object.keys(state.positions||{});
  const planets={};
  planetNames.forEach(k=>{planets[k]=Number(state.positions[k].toFixed(4));});
  return {
    birthDate: state.dt ? state.dt.toISOString() : null,
    latitude: state.lat,
    longitude: state.lon,
    lagna: {longitude:Number(state.lag.toFixed(4)), rasi:RASHIS[rasi(state.lag)][0]},
    moon: {longitude:Number(state.moon.toFixed(4)), rasi:RASHIS[rasi(state.moon)][0]},
    nakshatra: {name:NAK[state.ni.i][0], pada:state.ni.p},
    planets
  };
}
function renderAI(){
  if(!state){
    $("aiText").innerHTML=`<p>முதலில் உங்கள் பிறந்த தேதி, நேரம் மற்றும் இடத் தகவலை வைத்து <strong>ஜாதகம் கணக்கிடுங்கள்.</strong></p>`;
    return;
  }
  $("aiText").innerHTML=`<p><strong>ஜாதக Snapshot:</strong> ${RASHIS[rasi(state.lag)][0]} லக்னம் • ${RASHIS[rasi(state.moon)][0]} சந்திர ராசி • ${NAK[state.ni.i][0]} ${state.ni.p}ம் பாதம்.</p><p class="muted">இப்போது உங்கள் ஜாதகத் தரவுடன் AI ஜோதிடரிடம் கேள்வி கேட்கலாம்.</p>`;
}
...
function setChatBusy(busy){
  ...
  $("aiSend").textContent=busy?"…":"↑";
}
async function sendAIQuestion(){
  const q=$("aiInput").value.trim();
  if(!q) return;
  if(!state){alert("முதலில் உங்கள் ஜாதகத்தை கணக்கிடுங்கள்.");return;}
  // Prefer ai_config.js, but keep a built-in fallback for the APK.
  const apiUrl=(
    (window.AI_JOTHIDAR_CONFIG&&window.AI_JOTHIDAR_CONFIG.apiUrl) ||
    "https://quiet-pond-3e80.magicdigital821.workers.dev/chat"
  ).trim();
  if(!apiUrl || apiUrl.includes("YOUR-WORKER-NAME")){
    appendChat("assistant","AI Chat backend URL அமைக்கப்படவில்லை.");
    return;
  }
  appendChat("user",q);
  $("aiInput").value="";
  ...
  try{
    ...
    appendChat("assistant","AI சேவையை இப்போது தொடர்புகொள்ள முடியவில்லை. Internet அல்லது backend அமைப்பைச் சரிபார்க்கவும்.");
  }finally{
    setChatBusy(false);
    $("aiInput").focus();
  }
...
$("aiInput").addEventListener("keydown",e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendAIQuestion();}});
